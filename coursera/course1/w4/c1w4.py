...
if __name__ == "__main__":
    ...
    train_x = np.divide(train_x_flatten, 255.0, dtype=np.float32)
    test_x = np.divide(test_x_flatten, 255.0, dtype=np.float32)
    ...
    layer_configurations = [[7], [20, 7, 5]]

    for hidden_layer_dims in layer_configurations:
        print(f"\n\n>>> Testing with hidden todo of dimensions" f" {hidden_layer_dims}")
