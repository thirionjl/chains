...
class FullyConnected(ops.Op):
    ...
    def check_incoming_shapes(f: Shape, w: Shape, b: Shape):
        if f.ndim != 2:
            raise ValueError("Inputs of a fully connected layer should be a 2-D matrix")
        if w.ndim != 2:
            raise ValueError("Inputs of a fully connected layer should be a 2-D matrix")
        if not (b.is_column()):
        ...
        if w[1] != f[0]:
            raise ValueError(f"Number of columns of weight matrix should match number of rows of inputs matrix \
            but got {w[1]} and {f[0]}")
        if w[0] != b[0]:
            raise ValueError(f"Number of rows of weight matrix should match first dimension of bias \
            but got {w[0]} and {b[0]}")
    ...
    def compute(self, features: Tensor, weights: Tensor, bias: Tensor):
        ...
        self.output = weights @ features + bias  # bias.reshape(weights.shape[0], 1)  # Fixme: Broadcasting...
