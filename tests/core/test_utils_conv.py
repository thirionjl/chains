...
def test_all_dimensions_without_error():
    inputs = np.zeros((32, 3, 500, 400))
    filters = np.zeros((5, 3, 12, 10))
    assert uc._all_dimensions(inputs, filters) == (32, 3, 500, 400, 5, 12, 10)


def test_all_dimensions_with_non_matching_channels():
    with pytest.raises(ValueError) as ex:
        uc._all_dimensions(np.zeros((32, 4, 500, 400)),
                           np.zeros((5, 3, 12, 10)))
    assert str(ex.value) == 'Number of channels should be the same ' \
                            'in activations(4) and filters(3)'


def test_im2col_1channel_1example_stride1():
    inputs = np.arange(1, 10).reshape(1, 1, 3, 3)
    filters = np.zeros((1, 1, 2, 2))
    np.testing.assert_array_equal(uc.im2col(inputs, filters),
                                  np.array([[1, 2, 4, 5], [2, 3, 5, 6],
                                            [4, 5, 7, 8], [5, 6, 8, 9]]))


def test_im2col_same_convolution():
    inputs = np.arange(1, 17).reshape(1, 1, 4, 4)
    filters = np.zeros((1, 1, 3, 3))

    actual = uc.im2col(inputs, filters, padding=1)

    np.testing.assert_array_equal(actual[:, 0],
                                  np.array([0, 0, 0, 0, 1, 2, 0, 5, 6]).T)
    np.testing.assert_array_equal(actual[:, 5],
                                  np.array([1, 2, 3, 5, 6, 7, 9, 10, 11]).T)
    np.testing.assert_array_equal(actual[:, -1],
                                  np.array([11, 12, 0, 15, 16, 0, 0, 0, 0]).T)


def test_im2col_1channel_1example_stride2():
    inputs = np.arange(1, 17).reshape(1, 1, 4, 4)
    filters = np.zeros((1, 1, 2, 2))
    np.testing.assert_array_equal(uc.im2col(inputs, filters, stride=2),
...
def test_im2col_multi_channel_multi_example_stride1():
    ...
    np.testing.assert_array_equal(uc.im2col(inputs, filters), expected)


def test_im2col():
    inputs = np.zeros((2, 3, 3, 3), dtype=np.int64)
    filters = np.zeros((1, 3, 2, 2))
    ...
    np.testing.assert_array_equal(expected,
                                  uc.col2im(cols, inputs, filters))


def test_im2col_padding():
    inputs = np.zeros((2, 2, 2, 2), dtype=np.int64)
    filters = np.zeros((1, 2, 2, 2))
    cols = np.ones(shape=(2 * 2 * 2, 9 * 2), dtype=np.int64)
    expected = np.ones(shape=(2, 2, 2, 2)) * 4
    np.testing.assert_array_equal(expected,
                                  uc.col2im(cols, inputs, filters, padding=1))


def test_im2col_stride():
    inputs = np.zeros((1, 1, 5, 5), dtype=np.int64)
    filters = np.zeros((1, 1, 3, 3))
    ...
    actual = uc.col2im(cols, inputs, filters, stride=2)
