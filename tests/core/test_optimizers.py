...
class DummyGraph(Graph):
    def __init__(self):
        ...
        self.b2 = initialized_var(
            "b2", array([[-0.87785842], [0.04221375], [0.58281521]])
        )
...
def test_momentum_coursera():
    ...
    assert_allclose(g.b1.value, array([[1.74493465], [-0.76027113]]))
    ...
    assert_allclose(
        g.b2.value, array([[-0.87809283], [0.04055394], [0.58207317]]), atol=1e-8
    )
    ...
    assert_allclose(optimizer.v[g.b1], array([[-0.01228902], [-0.09357694]]), atol=1e-8)
    assert_allclose(
        optimizer.v[g.w2],
        array(
            [
    ...
    assert_allclose(
        optimizer.v[g.b2], array([[0.02344157], [0.16598022], [0.07420442]])
    )
...
def test_adam_coursera():
    ...
    assert_allclose(g.b2.value, array([[-0.88529979], [0.03477238], [0.57537385]]))
    ...
    assert_allclose(optimizer.s[g.b1], array([[1.51020075e-05], [8.75664434e-04]]))
    assert_allclose(
        optimizer.s[g.w2],
        array(
            [
    ...
    assert_allclose(
        optimizer.s[g.b2], array([[5.49507194e-05], [2.75494327e-03], [5.50629536e-04]])
    )
