...
from chains.front.network import Sequence, Dense, BatchNorm, ReLu, SoftmaxClassifier
...
def test_backup_network(tmpdir):
    ...
    model = Model(
        ...
        training=MiniBatchTraining(
            batch_size=32, optimizer=AdamOptimizer(0.0001), listener=TrainListener()
        ),
    )

    # Train
    model.train(
        test_x.astype(dtype=np.float32), test_y.astype(dtype=np.int16), epochs=5
    )
