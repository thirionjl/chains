...
class Dim:
    ...
    def is_broadcast_compatible(self, other):
        return (
                ...
                or other.value == 1
                or (
                        self.is_concrete() and other.is_concrete() and self.value == other.value
                )
        )
...
class StaticShape(tuple):
    ...
    @classmethod
    def from_tuple(cls, t: Iterable[int]):
        return cls(*t)
    ...
    def is_broadcast_compatible(self, other):
        return all(
            m.is_broadcast_compatible(n) for m, n in self._zip_dimensions(self, other)
        )

    def reduce_along_axis(self, axis: Union[Iterable[int], int], keep_dims=False):
        ...
        if keep_dims:
            ...
            return StaticShape.from_tuple(d for d, keep in zip(self, squeezed) if keep)

    @staticmethod
    def _zip_dimensions(a, b):
        reversed_zip = list(zip_longest(reversed(a), reversed(b), fillvalue=Dim.of(1)))
        return reversed(reversed_zip)

    def broadcast(self, other):
        args = (m.broadcast(n) for m, n in self._zip_dimensions(self, other))
    ...
    def check_axis_index(self, axis: int) -> None:
        if axis >= 0 and not (0 <= axis < self.ndim):
            raise ValueError(f"axis is out of bounds of shape {self} " f"got {axis}")
        if axis < 0 and not (-self.ndim <= axis < 0):
            raise ValueError(f"axis is out of bounds of shape {self} " f"got {axis}")

    def transpose(self, *args: int) -> "StaticShape":
