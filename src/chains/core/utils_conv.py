...
def _all_dimensions(activations_shape: Shape, filters_shape: Shape):
    ...
    if len(filters_shape) != 4:
        raise ValueError(
            f"filter_shape should be a dimensional "
...
def clip_positions_count(nh, nw, fh, fw, padding=0, stride=1):
    ...
    if padding >= fh or padding >= fw:
        raise ValueError(
            f"Padding({padding}) should be strictly smaller than"
    ...
    if height % stride != 0:
        raise ValueError(
            f"Padded height ({height}) should be divisible by "
            f"stride ({stride} but is not"
        )
    if width % stride != 0:
        raise ValueError(
            f"Padded width ({width}) should be divisible by "
            f"stride ({stride} but is not"
        )
