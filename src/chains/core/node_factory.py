...
def l2_norm_regularizer(lambd, batch_size, weight_matrices_array, name=None):
    ...
    return Node(
        L2NormRegularization(lambd=lambd),
...
def fully_connected(
    inputs: Node, weights: Node, bias: Node, *, first_layer=False, name=None
) -> Node:
    return Node(FullyConnected(not first_layer), [inputs, weights, bias], name=name)
...
def batch_norm_predict(
    batch_norm: Node, inputs: Node, beta: Node, gamma: Node, name=None
) -> Node:
