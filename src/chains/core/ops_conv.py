...
class Conv2D(Op):
    def __init__(
        self, feature_derivative=True, padding=0, stride=1, conv_format=uc.NCHW
    ):
        super().__init__()
    ...
    def check_incoming_shapes(self, features: Shape, filters: Shape, b: Shape):
        ...
        if filters.ndim != 4:
            raise ValueError(
                f"filters should be a 4 dimensional tensor "
        ...
        if c != cc:
            raise ValueError(
                f"Number of channels should be the same "
        ...
        if d != b[0]:
            raise ValueError(
                f"Number of bias should match number of filters "
                f"but got {b[0]} and {d}"
            )
    ...
    def compute(self, features: NdArrayLike, filters: NdArrayLike, bias: NdArrayLike):
        ...
        self.xc = uc.im2col(
            self.features, self.filters.shape, self.padding, self.stride
        )
    ...
    def partials(self, d_output):
        ...
        assert dd == d, (
            f"Number of filters in input({d}) is "
        ...
        dx = uc.col2im(
            dxc, self.features.shape, self.filters.shape, self.padding, self.stride
        )

        return self.conv_format.nchw_inv(dx), self.conv_format.dchw_inv(df), db

    def _reshape_out(self, zc: ndarray):
        """Reshapes ZC output to Z"""
        m, c, nh, nw, d, fh, fw = self._dimensions()
        cnt_h, cnt_w = uc.clip_positions_count(
            nh, nw, fh, fw, self.padding, self.stride
        )
