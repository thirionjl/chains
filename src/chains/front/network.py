...
class Sequence(Network):
    def __init__(
        ...
        if regularizer is not None:
            cost_graph = regularizer.append(
                self.layer_names, cost_graph, regularizable_vars, self.inputs
            )
...
class SequenceElement(abc.ABC):
    ...
    def prepare_names(self, name_generator: NameGenerator):
        self.layer_name = name_generator.generate(self.__class__.__name__).lower()
...
class Dense(Layer):
    ...
    def __init__(
        ...
        self.weight_initializer = (
            ...
            else weight_initializer
        )
        self.bias_initializer = (
            self.default_bias_initializer
    ...
    def do_append(self, pos, cost_g, predict_g):
        ...
        predict_fc = f.fully_connected(
            predict_g, w, b, first_layer=(pos == 0), name=self.layer_name + "_p"
        )
...
class Dropout(Layer):
    ...
    def do_append(self, pos, cost_graph, predict_graph):
        return (
            f.dropout(self.keep_prob, cost_graph, name=self.layer_name),
            predict_graph,
            [],
        )
...
class BatchNorm(Layer):
    ...
    def __init__(self, beta_initializer=None, gamma_initializer=None, namespace=None):
        ...
        self.gamma_initializer = (
            self.default_gamma_initializer
    ...
    def do_append(self, pos, cost_graph, predict_graph):
        ...
        bnp = f.batch_norm_predict(
            bnt.op, predict_graph, beta, gamma, name=self.layer_name + "_p"
        )
...
class Classifier(SequenceElement, abc.ABC):
    ...
    def append(
        self, name_generator, cost_graph: Node, predict_graph: Node, labels: Node
    ):
        self.prepare_names(name_generator)
...
class SigmoidBinaryClassifier(Classifier):
    ...
    def do_append(self, cost_graph: Node, predict_graph: Node, labels: Node):
        ...
        cross_entropy = f.sigmoid_cross_entropy(
            cost_graph, labels, name=self.layer_name
        )
...
class SoftmaxClassifier(Classifier):  # TODO axis
    ...
    def do_append(self, cost_graph: Node, predict_graph: Node, labels: Node):
        entropy = f.softmax_cross_entropy(cost_graph, labels, name=self.layer_name)
...
class L2Regularizer(Regularizer):
    ...
    def do_append(self, cost_graph: Node, vars, inputs):
        ...
        reg = f.l2_norm_regularizer(self.lambd, dim, vars, name=self.layer_name)
        if self.lambd > 0:
